// Inline HIROTA wordmark ("unflavored" horizontal logo). Inlined instead of
// <img src> for the same reason as HirotaVerticalLogo/KarateGiVector: the stroke
// must stay a constant ~1px at any rendered size, matching the site borders.
//   1. stroke uses currentColor -- set the colour with a text-* utility (text-black).
//      It stays SOLID; translucency comes from GROUP opacity on the <g> wrapper,
//      NOT a translucent stroke, so overlapping strokes never accumulate alpha.
//   2. vector-effect="non-scaling-stroke" keeps the stroke a constant width no
//      matter the element's size (an <img> would scale the stroke with it).
//      strokeWidth="1" matches the site borders.
// Opacity defaults to the shared --svg-opacity (0.5) but can be overridden per
// use with the `opacity` prop (the admin landing dims it further).
type Props = {
  className?: string;
  /** Group opacity (0–1). Defaults to the site's --svg-opacity token (0.5). */
  opacity?: number;
  "aria-label"?: string;
};

export function HirotaLogo({ className, opacity, ...rest }: Props) {
  return (
    <svg
      viewBox="0 0 405.25 53.62"
      fill="none"
      role="img"
      className={className}
      {...rest}
    >
      {/* Solid strokes + group opacity: overlaps do not accumulate alpha. */}
      <g style={{ opacity: opacity ?? "var(--svg-opacity)" }}>
        <path
          d="M34.76,53.25H.84S33.48,31.84,28.22.25h16.5l-9.96,53Z"
          stroke="currentColor"
          strokeWidth="1"
          strokeMiterlimit="10"
        ...
        />
        <path
          d="M48.78.25h33.92s-32.65,21.41-27.37,53h-16.5L48.78.25Z"
          stroke="currentColor"
          strokeWidth="1"
          strokeMiterlimit="10"
          vectorEffect="non-scaling-stroke"
